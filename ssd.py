...
# Define transformation to normalize the image for SSD
...
def process_image(image, threshold=0.5):
    """
    Process the image using SSD and return the detections.
...
def visualize_detections(image, boxes, labels, scores):
    ...
    :param image: The input image in BGR format.
    :param boxes: Bounding boxes from the SSD output.
    :param labels: Class labels from the SSD output.
    :param scores: Confidence scores from the SSD output.
    """
...
def run_video_detection(video_source=0, threshold=0.5, log_file="ssd_log.json"):
    """
    Run SSD detection on a video source (webcam or video file).
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_file", type=str, default="ssd_log.json", help="File to log detections and inference times")
    
    args = parser.parse_args()

    # Test the SSD model on video
